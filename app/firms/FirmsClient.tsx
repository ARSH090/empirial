            ...
            <AnimatePresence mode="popLayout">
              {processedFirms.length > 0 ? (
                processedFirms.map((firm) => {
                  ...
                  // Filter platform logos ensuring category separation
                  const platformKeys = (firm.platform_ids || []).filter((pid) => {
                    const pInfo = PLATFORM_DATA[pid];
                    if (!pInfo) return false;
                    if (firm.category === 'futures') {
                      return pInfo.type === 'futures';
                    }
                    return pInfo.type === 'forex';
                  });
                  ...
                  return (
                    <motion.div
                      ...
                      className={`relative rounded-2xl transition-all duration-300 border ${
                        isExpanded
                          ? 'border-black dark:border-white shadow-[0_0_20px_rgba(0,0,0,0.14)] dark:shadow-[0_0_25px_rgba(255,255,255,0.18)] bg-white/65 dark:bg-card backdrop-blur-md'
                    ...
                    >
                      ...
                      <div className="p-4 sm:p-5">
                        ...
                        <div className="hidden lg:grid grid-cols-12 gap-4 items-center">
                          ...
                          {/* 4. Platforms Logos (Smooth Micro-Animation on category change) */}
                          <div className="col-span-2 flex items-center justify-center gap-2 flex-wrap">
                            {platformKeys.length > 0 ? (
                                  ...
                                  <Tooltip key={pid}>
                                    <TooltipTrigger asChild>
                                      <motion.div
                                        ...
                                        className="w-7 h-7 rounded-md bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center p-1 shadow-2xs hover:scale-110 transition-transform"
                                      >
                                        <img
                                          src={p.logo}
                                          alt={p.name}
                              ...
                              <span className="text-xs text-muted-foreground">{firm.platforms}</span>
                            )}
                          </div>
                          ...
                          {/* 6. Action Buttons & Code Mechanism */}
                          <div className="col-span-2 flex items-center justify-end gap-2">
                            {/* Code Empire / Discount Button with Shake Animation */}
                            <motion.button
                              type="button"
                            ...
                            >
                              {isCopiedNow ? (
                                ...
                                <>
                                  ...
                                  <span>{firm.discount_label_custom || 'Code EMPIRE'}</span>
                                </>
                              )}
                            </motion.button>
                        ...
                        <div className="lg:hidden flex flex-col space-y-3.5">
                          ...
                          {/* Spec Badges on Mobile */}
                          <div className="grid grid-cols-2 gap-2 text-xs py-1 border-t border-b border-zinc-100 dark:border-zinc-800">
                            <div>
                              <span className="text-[10px] text-muted-foreground uppercase font-semibold">Max Allocation</span>
                              <div className="font-extrabold text-foreground">{firm.max_allocation}</div>
                            </div>
                            <div>
                              <span className="text-[10px] text-muted-foreground uppercase font-semibold">Profit Split</span>
                              <div className="font-extrabold text-foreground">{firm.profit_split_custom}</div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {platformKeys.map((pid) => {
                              ...
                              return (
                                <div
                                  key={pid}
                                  className="w-6 h-6 rounded-md bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center p-0.5"
                                >
                                  <img src={p.logo} alt={p.name} className="h-4 w-4 object-contain rounded-xs" />
                                </div>
                              );
                            })}
                          </div>
                          ...
                          <div className="flex items-center gap-2 pt-1">
                            ...
                            >
                              {isCopiedNow ? 'Copied!' : hasCopied ? 'Code Copied ✓' : (firm.discount_label_custom || 'Code EMPIRE')}
                            </motion.button>
                      ...
                      <AnimatePresence>
                        {isExpanded && (
                          <motion.div
                            ...
                            exit={{ opacity: 0, height: 0 }}
                            transition={{ duration: 0.25, ease: 'easeInOut' }}
                            className="overflow-hidden border-t border-zinc-100 dark:border-zinc-800/80 bg-zinc-50/50 dark:bg-zinc-900/30 rounded-b-2xl"
                          >
                            <div className="p-4 sm:p-6 space-y-4">
                              {/* Expanded Detail Badges */}
                              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                {/* Country / HQ */}
                                <div className="p-3 rounded-xl bg-white dark:bg-card border border-zinc-200/80 dark:border-border flex items-start gap-2.5">
                                  ...
                                  <div>
                                    ...
                                    <span className="text-xs font-bold text-foreground">
                                      {firm.headquarters || 'Global Office'}
                                    </span>
                                  </div>
                                </div>
                                ...
                                <div className="p-3 rounded-xl bg-white dark:bg-card border border-zinc-200/80 dark:border-border flex items-start gap-2.5">
                                  ...
                                  <div>
                                    ...
                                    <span className="text-xs font-bold text-foreground">
                                      {firm.years_working || `Est. ${firm.founded_year || '2021'}`}
                                    </span>
                                  </div>
                                </div>
                              ...
                              {/* Evaluation Models & Description */}
                              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
                                {/* Models */}
                                <div className="flex items-center gap-2 flex-wrap">
                                  <span className="text-xs text-muted-foreground font-semibold">Evaluation Models:</span>
                                  {(firm.models || ['1-Step Challenge', '2-Step Evaluation', 'Instant Model']).map((model) => (
                                    <span
                                      key={model}
                                      className="px-2.5 py-1 rounded-md text-[11px] font-semibold bg-white dark:bg-card text-foreground border border-zinc-200 dark:border-border"
                                    >
                                      {model}
                                    </span>
                                  ))}
