...
export default function FirmProfilePage() {
  ...
  const firm = MOCK_FIRMS.find((f) => f.slug === slug) || MOCK_FIRMS[0];
  const firmChallenges = MOCK_CHALLENGES.filter((c) => c.firm_slug === firm.slug || c.firm_id === firm.id);
  const firmDeals = MOCK_DEALS.filter((d) => d.firm_slug === firm.slug || d.firm_id === firm.id);
  const firmReviews = MOCK_REVIEWS.filter((r) => r.firm_id === firm.id);
  const firmPayouts = MOCK_PAYOUTS.filter((p) => p.firm_id === firm.id);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      {/* Tab Content 1: Overview (Rules, Consistency, Specs, Restricted Countries) */}
      {activeTab === 'overview' && (
